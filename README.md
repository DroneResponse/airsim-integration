...
> [!NOTE]
> Only has been built using an Arm Mac to date. It may work on other Linux devices, but isn't guaranteed without modifications.
...
The gazebo pose sender and airsim pose receiver are two separate programs that communicate over UDP. This allows them to be run on separate machines or separate containers. If multiple drones exist in the gazebo simulation, the same number of drones will be spawned in AirSim. Each drone gets a unique name and will only receive its relevant poses.
...
To run all unit tests for either the sender or receiver, navigate to the `/build/test` dir in the relevant parent directory.
